...
const genrateLogoLine = () =>
{
    ...
    // const texture = new THREE.TextureLoader().load( 'src/img/Background-color image 1080p.png' );
    ...
    const tradeParticalTexture = tradeTextureLoader.load('/textures/ampersand2.jpeg')
...
loader.load( '/textures/perfect_ampersand.json', function ( font ) {
...
const particalTexture = textureLoader.load('/textures/ampersand2.jpeg')
...
const updateBrands = (event) => {
     ...
     coinParticals.scale.z = 1+window.scrollY*0.01
     coinParticals.scale.x = 1-window.scrollY*0.00001
        // coinParticals.scale.x = 0.5+(elapsedTime*2)
        coinParticals.scale.y = 1+window.scrollY*0.001
...
window.addEventListener( 'scroll', updateBrands );
